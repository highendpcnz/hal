import{C as e,D as t,E as n,N as r,O as i,P as a,_ as o,a as s,c,d as l,i as u,j as d,k as f,l as p,n as m,r as h,t as g}from"./UnrealBloomPass.js";var _={idle:{attend:0,vortex:0,chill:0,coherence:0,flow:.3,brightness:.5,bloom:.3},listening:{attend:1,vortex:0,chill:0,coherence:0,flow:.55,brightness:.85,bloom:.5},thinking:{attend:.15,vortex:1,chill:0,coherence:0,flow:1,brightness:.6,bloom:.34},speaking:{attend:.2,vortex:0,chill:0,coherence:1,flow:.45,brightness:.9,bloom:.5},denied:{attend:0,vortex:0,chill:1,coherence:0,flow:.04,brightness:.3,bloom:.12}},v={fetch:new l(16726820),execute:new l(16738847),search:new l(15279900),read:new l(16723232)},y=48e3,b=3,x=700,S=new a(1.15,.1,1.4),C=`
  attribute vec3 seed;
...
`,T=class{camera=new e(34,1,.1,80);timer=new d;composer;bloomPass;container;eye;geometry=new p;material;reducedMotionQuery=window.matchMedia(`(prefers-reduced-motion: reduce)`);renderer;scene=new i;targetPointer=new r;audioEnergy=0;destroyed=!1;domPollTimer=0;flowSpeedSeed=1;flowTime=0;frameId=0;missionCount=0;lagCool=0;pointer=new r;populationScale=1;pulseAge=99;resizeObserver;state=`idle`;stateObserver;targetAudioEnergy=0;tier=y;toolKind=null;constructor(e,i){this.container=e,this.eye=i,this.renderer=new s({alpha:!1,antialias:!1,powerPreference:`high-performance`}),this.renderer.domElement.className=`optic-webgl-canvas`,this.renderer.outputColorSpace=t,this.renderer.toneMapping=4,this.renderer.toneMappingExposure=.92,this.renderer.setClearColor(65793,1),this.container.appendChild(this.renderer.domElement),this.scene.fog=null,this.timer.connect(document),this.buildAttributes(),this.material=new f({uniforms:{uFlowTime:{value:0},uRealTime:{value:0},uTurbulence:{value:0},uAttend:{value:0},uVortex:{value:0},uChill:{value:0},uCoherence:{value:0},uShimmer:{value:0},uPulseAge:{value:99},uPulseColor:{value:new l(16723232)},uMissions:{value:0},uSpreadScale:{value:1},uSize:{value:1.9},uBrightness:{value:.5},uCoolBias:{value:0},uEyeCenter:{value:S.clone()}},vertexShader:C,fragmentShader:w,transparent:!0,depthWrite:!1,depthTest:!1,blending:2});let a=new n(this.geometry,this.material);a.frustumCulled=!1,this.scene.add(a),this.composer=new u(this.renderer),this.composer.addPass(new m(this.scene,this.camera)),this.bloomPass=new g(new r(1,1),.3,.7,.55),this.composer.addPass(this.bloomPass),this.composer.addPass(new h),this.seedFromHistory(),this.pollDom(),this.domPollTimer=window.setInterval(()=>this.pollDom(),5e3),this.resizeObserver=new ResizeObserver(()=>this.resize()),this.resizeObserver.observe(this.container),this.stateObserver=new MutationObserver(()=>this.syncStateFromDom()),this.stateObserver.observe(this.eye,{attributes:!0,attributeFilter:[`class`]}),window.addEventListener(`pointermove`,this.onPointerMove),window.addEventListener(`blur`,this.onPointerLeave),this.reducedMotionQuery.addEventListener(`change`,this.onMotionPreferenceChange),this.resize(),this.syncStateFromDom(),this.container.closest(`.eye-module`)?.classList.add(`webgl-ready`),this.frameId=requestAnimationFrame(this.animate)}setAudioEnergy(e){this.targetAudioEnergy=o.clamp(e,0,1)}setState(e){this.state=e,this.updateStatusLabel(e)}setToolKind(e){this.toolKind=e,e&&(this.pulseAge=0,this.material.uniforms.uPulseColor.value.copy(v[e]))}destroy(){this.destroyed||(this.destroyed=!0,cancelAnimationFrame(this.frameId),window.clearInterval(this.domPollTimer),this.resizeObserver.disconnect(),this.stateObserver.disconnect(),window.removeEventListener(`pointermove`,this.onPointerMove),window.removeEventListener(`blur`,this.onPointerLeave),this.reducedMotionQuery.removeEventListener(`change`,this.onMotionPreferenceChange),this.timer.dispose(),this.geometry.dispose(),this.material.dispose(),this.composer.dispose(),this.renderer.dispose(),this.renderer.domElement.remove(),this.container.closest(`.eye-module`)?.classList.remove(`webgl-ready`))}buildAttributes(){let e=new Float32Array(y*3),t=new Float32Array(y*3),n=new Float32Array(y*3),r=new Float32Array(y),i=new Float32Array(y);for(let e=0;e<y;e+=1){t[e*3]=Math.random(),t[e*3+1]=Math.random(),t[e*3+2]=Math.random();let a=Math.random()<.08,o=a?Math.random()*.22:.45+Math.sqrt(Math.random())*1.35,s=Math.random()*Math.PI*2;n[e*3]=Math.cos(s)*o,n[e*3+1]=Math.sin(s)*o,n[e*3+2]=(Math.random()-.5)*.14,r[e]=a?1:Math.max(0,.65-o*.3)+Math.random()*.15,i[e]=e<b*x?Math.floor(e/x)+1:0}this.geometry.setAttribute(`position`,new c(e,3)),this.geometry.setAttribute(`seed`,new c(t,3)),this.geometry.setAttribute(`formation`,new c(n,3)),this.geometry.setAttribute(`heat`,new c(r,1)),this.geometry.setAttribute(`group`,new c(i,1))}seedFromHistory(){let e=document.querySelectorAll(`#mlog-entries > *`).length;this.populationScale=.7+Math.min(e,80)/80*.3,this.flowSpeedSeed=1.15-Math.min(e,80)/80*.3}pollDom(){try{this.missionCount=Math.min(document.querySelectorAll(`#mission-cards .mission-card`).length,b);let e=document.getElementById(`telem-lag`)?.textContent??``,t=parseInt(e,10);this.lagCool=Number.isFinite(t)?o.clamp((t-4e3)/12e3,0,1):0}catch{this.missionCount=0,this.lagCool=0}}resize(){let{width:e,height:t}=this.container.getBoundingClientRect();if(e<2||t<2)return;this.renderer.setPixelRatio(Math.min(window.devicePixelRatio||1,e>=1101?1.75:e>=761?1.5:1.25)),this.tier=e>=1101?4e4:e>=761?24e3:1e4;let n=b*x,r=Math.floor(Math.min(this.tier,y-n)*this.populationScale);this.geometry.setDrawRange(0,n+r),this.camera.aspect=e/t;let i=this.camera.aspect<1.05;this.material.uniforms.uSpreadScale.value=i?.55:1;let a=i?7.6:8.2,s=i?6.4:12.4,c=o.degToRad(this.camera.fov/2),l=a/2/Math.tan(c),u=s/2/(Math.tan(c)*this.camera.aspect);this.camera.position.set(0,0,Math.max(l,u)),this.camera.lookAt(0,0,0),this.camera.updateProjectionMatrix(),this.renderer.setSize(e,t,!1),this.composer.setSize(e,t)}syncStateFromDom(){let e=`idle`;this.eye.classList.contains(`denied`)?e=`denied`:this.eye.classList.contains(`listening`)?e=`listening`:this.eye.classList.contains(`speaking`)?e=`speaking`:this.eye.classList.contains(`thinking`)&&(e=`thinking`),this.setState(e);let t=this.eye.classList.contains(`kind-execute`)?`execute`:this.eye.classList.contains(`kind-fetch`)?`fetch`:this.eye.classList.contains(`kind-search`)?`search`:this.eye.classList.contains(`kind-read`)?`read`:null;t!==this.toolKind&&this.setToolKind(t)}updateStatusLabel(e){let t=document.getElementById(`optic-status-label`);t&&(t.textContent={idle:`Standby`,listening:`Listening`,thinking:`Thinking`,speaking:`Speaking`,denied:`Permission denied`}[e],t.dataset.state=e)}onPointerMove=e=>{this.reducedMotionQuery.matches||this.targetPointer.set(o.clamp(e.clientX/window.innerWidth*2-1,-1,1),o.clamp(-(e.clientY/window.innerHeight*2-1),-1,1))};onPointerLeave=()=>{this.targetPointer.set(0,0)};onMotionPreferenceChange=()=>{this.reducedMotionQuery.matches&&(this.targetPointer.set(0,0),this.pointer.set(0,0))};animate=e=>{if(this.destroyed)return;this.frameId=requestAnimationFrame(this.animate),this.timer.update(e);let t=Math.min(this.timer.getDelta(),.05),n=this.timer.getElapsed(),r=_[this.state],i=this.reducedMotionQuery.matches;this.audioEnergy=o.damp(this.audioEnergy,this.targetAudioEnergy,8,t),this.targetAudioEnergy=o.damp(this.targetAudioEnergy,0,2.6,t),this.pointer.x=o.damp(this.pointer.x,this.targetPointer.x,5,t),this.pointer.y=o.damp(this.pointer.y,this.targetPointer.y,5,t),i||(this.flowTime+=t*r.flow*this.flowSpeedSeed),this.pulseAge+=t;let a=this.material.uniforms;a.uFlowTime.value=this.flowTime,a.uRealTime.value=i?0:n,a.uTurbulence.value=o.damp(a.uTurbulence.value,i?0:this.audioEnergy,7,t);let s=r.coherence>a.uCoherence.value?2.6:9;a.uCoherence.value=o.damp(a.uCoherence.value,r.coherence,i?20:s,t),a.uAttend.value=o.damp(a.uAttend.value,r.attend,4,t),a.uVortex.value=o.damp(a.uVortex.value,r.vortex,4,t),a.uChill.value=o.damp(a.uChill.value,r.chill,r.chill>0?10:4,t),a.uShimmer.value=this.state===`speaking`&&!i?this.audioEnergy:0,a.uPulseAge.value=i?this.pulseAge<.6?.4:99:this.pulseAge,a.uMissions.value=this.missionCount,a.uBrightness.value=o.damp(a.uBrightness.value,r.brightness+this.audioEnergy*.4,5,t),a.uCoolBias.value=o.damp(a.uCoolBias.value,this.lagCool,2,t),this.camera.position.x=o.damp(this.camera.position.x,this.pointer.x*.5,4,t),this.camera.position.y=o.damp(this.camera.position.y,this.pointer.y*.32,4,t),this.camera.lookAt(0,0,0),this.bloomPass.strength=o.damp(this.bloomPass.strength,r.bloom+this.audioEnergy*.3,6,t),this.composer.render()}};function E(e,t){let n=new T(e,t);return{destroy:()=>n.destroy(),setAudioEnergy:e=>n.setAudioEnergy(e),setState:e=>n.setState(e),setToolKind:e=>n.setToolKind(e)}}export{E as createOptic};
